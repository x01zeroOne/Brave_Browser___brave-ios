...
struct ReaderModeUtils {
  ...
  static func simplifyDomain(_ domain: String) -> String {
    domainPrefixesToSimplify.first { domain.hasPrefix($0) }.map {
  ...
  static func generateReaderContent(
    ...
    return tmpl
      .replacingOccurrences(of: "%READER-TITLE-NONCE%", with: titleNonce) // This MUST be the first line/replacement!
      ...
      .replacingOccurrences(
        of: "%READER-TITLE%",
        with: readabilityResult.title.javaScriptEscapedString?.unquotedIfNecessary ?? readabilityResult.title
          .htmlEntityEncodedString
      )
      .replacingOccurrences(
        of: "%READER-CREDITS%",
        with: readabilityResult.credits.javaScriptEscapedString?.unquotedIfNecessary ?? readabilityResult.credits
          .htmlEntityEncodedString
      )
