  ...
  static func register(_ webServer: WebServer, profile: Profile) {
    ...
    webServer
      .registerHandlerForMethod(
          ...
          if let url = URL(string: url), url.isWebPage() {
            do {
              ...
              if let html = ReaderModeUtils.generateReaderContent(
                ...
                response.setValue(
                  "default-src 'none'; img-src *; style-src http://localhost:* '\(readerModeStyleHash)'; font-src http://localhost:*; script-src 'nonce-\(setTitleNonce)'",
        ...
        ) // TODO: Needs a proper error page
      }
  }
}
