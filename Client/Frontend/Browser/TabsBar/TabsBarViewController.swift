...
class TabsBarViewController: UIViewController {
  ...
  private func addScrollHint(for side: HintSide, maskLayer: CAGradientLayer) {
    ...
    guard let barsColor = collectionView.backgroundColor?.resolvedColor(with: traitCollection) ?? view.backgroundColor
    else {
...
extension TabsBarViewController: UICollectionViewDataSource {
  ...
  func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
    ...
    guard let tab = tabList[indexPath.row] else {
      return cell
    }
    ...
    cell.closeTabCallback = { [weak self] tab in
      guard let strongSelf = self, let tabManager = strongSelf.tabManager,
            let previousIndex = strongSelf.tabList.index(of: tab) else {
        return
      }
  ...
  func collectionView(
    ...
    guard let to = tabs.firstIndex(where: { $0 === toTab }) else {
      return
    }
    ...
    guard let selectedTab = tabList[destinationIndexPath.row] else {
      return
    }
