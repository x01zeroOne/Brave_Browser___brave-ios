...
class TestSQLiteLogins: XCTestCase {
  ...
  func testGetOrder() {
    ...
    addLogin(login) >>> { self.addLogin(login2) } >>> getLoginsFor(login.protectionSpace, expected: [login2, login]) >>>
      done(expectation)
  ...
  func testUpdateLogin() {
    ...
    addLogin(login) >>> { self.updateLogin(updated) } >>> getLoginsFor(login.protectionSpace, expected: [updated]) >>>
      done(expectation)
  ...
  func testSearchLogins() {
    ...
    func checkAllLogins() -> Success {
      logins.getAllLogins() >>== { results in
    ...
    func checkSearchHostnames() -> Success {
      logins.searchLoginsWithQuery("pha") >>== { results in
    ...
    func checkSearchUsernames() -> Success {
      logins.searchLoginsWithQuery("username") >>== { results in
        XCTAssertEqual(results.count, 4)
    ...
    func checkSearchPasswords() -> Success {
      logins.searchLoginsWithQuery("pass") >>== { results in
        XCTAssertEqual(results.count, 4)
