...
struct PortfolioState: Equatable {
  ...
  var selectedToken: BraveWallet.BlockchainToken?
}
...
enum PortfolioAction: BindableAction, Equatable {
  ...
  case assetTapped(BraveWallet.BlockchainToken)
...
> = .init { state, action, environment in
  ...
  case .addAccount:
    return .fireAndForget {
      environment.keyringService.addAccount("Test") { _ in
      }
    }
  ...
  case .update:
    ...
    return .merge(
      .future { callback in
        Task {
          let chainId = await environment.rpcService.chainId()
      ...
      .future { [selectedTimeframe = state.selectedTimeframe] callback in
        Task { @MainActor in
          ...
          let assets = await environment.walletService.userAssets(chainId)
            .filter(\.visible)
          let keyring = await environment.keyringService.keyringInfo(BraveWallet.DefaultKeyringId)
          ...
          let fetchBalanceTask = Task { () -> [String: Double] in
            ...
            for account in keyring.accountInfos {
              for token in assets {
                if let balance = await environment.rpcService.balance(for: token, in: account) {
          ...
          let fetchPricesTask = Task { () -> [String: String] in
            let (success, prices) = await Task { @MainActor in
              return await environment.assetRatioService
                .price(
          ...
          let fetchHistoryTask = Task { () -> [BalanceTimePrice] in
            ...
            for token in assets {
              let (success, values) = await Task { @MainActor in
                return await environment.assetRatioService.priceHistory(token.symbol.lowercased(), vsAsset: "usd", timeframe: selectedTimeframe)
              }.value
              if success {
