  ...
  func updateGasValue(for transaction: BraveWallet.TransactionInfo) {
    ...
    if !self.state.gasValue.isEmpty {
      rpcService.chainId(.eth) { [weak self] chainId in
        self?.rpcService
          .balance(transaction.fromAddress, coin: .eth, chainId: chainId) { [weak self] weiBalance, status, _ in
        ...
        self.blockchainRegistry.allTokens(chainId) { tokens in
          self.walletService.userAssets(chainId) { userAssets in
            let allTokens = tokens + userAssets.filter { asset in
              ...
              !tokens
                .contains(where: {
                  $0.contractAddress(in: selectedChain).caseInsensitiveCompare(asset.contractAddress) == .orderedSame
                })
            }
            ...
            case .erc20Approve:
              ...
              if let token = allTokens.first(where: {
                ...
                self.state.value = formatter.decimalString(
                  for: approvalValue,
                  radix: .hex,
