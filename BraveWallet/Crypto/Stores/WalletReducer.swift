...
struct WalletEnvironment {
  ...
  let rpcServiceObserver: Effect<NetworkObserverAction, Never>
  let walletService: BraveWalletBraveWalletService
  let assetRatioService: BraveWalletAssetRatioService
  let swapController: BraveWalletSwapService
  let tokenRegistry: BraveWalletBlockchainRegistry
  let transactionService: BraveWalletEthTxService
...
let walletReducer = Reducer<WalletState, WalletAction, WalletEnvironment>.combine(
  unlockReducer
    .pullback(state: /WalletState.unlock, action: /WalletAction.unlock, environment: { environment in
      return UnlockEnvironment(
        keyringService: environment.keyringService,
  ...
  onboardingReducer
    .pullback(state: /WalletState.onboarding, action: /WalletAction.onboarding, environment: { environment in
      return OnboardingEnvironment(keyringService: environment.keyringService)
    }),
    ...
    case .onAppear:
      return .merge(
        .fireAndForget {
          ...
          environment.rpcService.chainId { chainId in
            if chainId.isEmpty {
              environment.rpcService.setNetwork(BraveWallet.MainnetChainId) { _ in }
            }
          }
        },
        // Collect keyring info for setting up base state
        .future { completion in
          environment.keyringService.defaultKeyringInfo { keyringInfo in
...
class KeyringObserver: BraveWalletKeyringServiceObserver {
...
class TransactionObserver: BraveWalletEthTxServiceObserver {
...
extension BraveWalletEthTxService {
...
class NetworkObserver: BraveWalletJsonRpcServiceObserver {
  struct CancelId: Hashable { }
...
struct NetworkEnvironment {
  var rpcService: BraveWalletJsonRpcService
...
> { state, action, env in
  ...
  case .fetchSelectedNetwork:
    return .merge(
      .future { callback in
        env.rpcService.network { chain in
  ...
  case .changeSelectedNetwork(let chain):
    ...
    return .fireAndForget {
      env.rpcService.setNetwork(chain.chainId) { _ in }
    }
