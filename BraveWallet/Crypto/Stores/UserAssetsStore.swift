...
public class UserAssetsStore: ObservableObject {
  ...
  func tokenInfo(by contractAddress: String, completion: @escaping (BraveWallet.BlockchainToken?) -> Void) {
    if let assetStore = assetStores
      .first(where: { $0.token.contractAddress.lowercased() == contractAddress.lowercased() }) {
      // First check user's visible assets
      completion(assetStore.token)
    }
    else if let token = allTokens
      .first(where: { $0.contractAddress.lowercased() == contractAddress.lowercased() }) {
      // Then check full tokens list
      completion(token)
