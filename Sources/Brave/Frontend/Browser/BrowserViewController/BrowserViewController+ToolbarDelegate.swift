      ...
      addToPlaylist(item: playlistItem) { [weak self] didAddItem in
        ...
        if didAddItem {
          ...
          DispatchQueue.main.async { [self] in
            let popover = self.createPlaylistPopover(tab: tab)
    ...
    case .openInPlaylist:
      DispatchQueue.main.async {
        if let webView = tab.webView {
          PlaylistScriptHandler.getCurrentTime(webView: webView, nodeTag: info.tagId) { [weak self] currentTime in
            self?.openPlaylist(tab: tab, item: info, playbackOffset: currentTime)
          }
        } else {
          self.openPlaylist(tab: tab, item: info, playbackOffset: 0.0)
        }
      }
    case .remove:
      DispatchQueue.main.async {
