  ...
  private func menuSwappingAction(
    ...
    if #available(iOS 16.0, *) {
      ...
      action = UIAction(title: title, image: image, attributes: attributes, handler: UIAction.deferredActionHandler { _ in
        let menu = handler()
        self.menu = menu
        self.contextMenuInteraction?.perform(NSSelectorFromString("_presentMenuAtLocation:"), with: CGPoint.zero)
      })
    }
    return action
  }
  ...
  private func defaultMenu(for info: PlaylistInfo?) -> UIMenu {
    return UIMenu(title: "", image: nil, identifier: nil, options: [], children: [
      UIDeferredMenuElement.uncached { handler in
        ...
        handler([
          UIAction(title: "Added to \(item?.playlistFolder?.title ?? "Playlist")", image: UIImage(braveSystemNamed: "leo.check.circle-filled")?.withTintColor(.braveSuccessLabel).withRenderingMode(.alwaysOriginal), attributes: .disabled, handler: { _ in }),
        ])
      },
      UIMenu(title: "", subtitle: nil, image: nil, identifier: nil, options: .displayInline, children: [
        UIAction(title: "Open In Playlist", image: UIImage(braveSystemNamed: "leo.product.playlist"), handler: { _ in
          self.menuActionHandler?(.openInPlaylist)
        }),
        UIAction(title: "Change Folder", image: UIImage(braveSystemNamed: "leo.folder.exchange"), handler: { _ in
          self.menuActionHandler?(.changeFolders)
        }),
        menuSwappingAction(title: "Remove", image: UIImage(braveSystemNamed: "leo.trash"), attributes: .destructive, handler: {
  ...
  private func deletedMenu(for info: PlaylistInfo?) -> UIMenu {
    return UIMenu(title: "", image: nil, identifier: nil, options: [], children: [
      UIDeferredMenuElement.uncached { handler in
        ...
        handler([
          UIAction(title: "Removed from \(item?.playlistFolder?.title ?? "Playlist")", image: UIImage(braveSystemNamed: "leo.trash"), attributes: .disabled, handler: { _ in }),
          UIMenu(title: "", subtitle: nil, image: nil, identifier: nil, options: .displayInline, children: [
            self.menuSwappingAction(title: "Undo", image: UIImage(braveSystemNamed: "leo.arrow.back"), handler: {
              // Re-add to playlist
